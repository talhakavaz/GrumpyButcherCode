...
namespace DynamicMeshCutter
{
    ...
    public class MouseBehaviour : CutterBehaviour
    {
        ...
        private void Cut()
        {
            ...
            //Debug.Log("<color=yellow> plane name:: </color>" + plane.ToString());

            var roots = UnityEngine.SceneManagement.SceneManager.GetActiveScene().GetRootGameObjects();
            //Debug.Log("<color=yellow> roots name:: </color>" + roots.ToString());
            foreach (var root in roots)
            {
                if (!root.activeInHierarchy)
                    continue;
                var targets = root.GetComponentsInChildren<MeshTarget>();
                //Debug.Log("<color=yellow> targets name:: </color>" + targets.ToString());
                foreach (var target in targets)
                {
                    Cut(target, _to, plane.normal, null, OnCreated);
                    //Debug.Log("<color=yellow> target name:: </color>" + target.ToString());
                }
            }
        }
