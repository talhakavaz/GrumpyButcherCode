    ...
    */
    ...
    public abstract class CutterBehaviour : MonoBehaviour
    {
        ...
        protected virtual void CreateGameObjects(Info info)
        {
            ...
            // Find or create the single master parent "SlicedMeshesMasterParent"
            GameObject masterParent = GameObject.Find("SlicedMeshesMasterParent");
            if (masterParent == null)
            {
                masterParent = new GameObject("SlicedMeshesMasterParent");
            }

            // Parent all created objects to the master parent
            for (int i = 0; i < creationInfo.CreatedObjects.Length; i++)
            {
                if (creationInfo.CreatedObjects[i] != null)
                {
                    creationInfo.CreatedObjects[i].transform.SetParent(masterParent.transform, true);
                }
                else
                {
                    Debug.Log("Dynamic Mesh Cutter: Cut suppressed creation of object due to VertexCreationThreshold. Make sure you handle NullReferenceExceptions!");
                }
            }
